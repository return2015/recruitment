...
/*import com.returnsoft.resource.service.AreaInterface;
import com.returnsoft.resource.service.CandidateInterface;
import com.returnsoft.resource.service.CountryInterface;
import com.returnsoft.resource.service.DepartmentInterface;
import com.returnsoft.resource.service.DistrictInterface;
import com.returnsoft.resource.service.EducationLevelInterface;
import com.returnsoft.resource.service.InterviewInterface;
import com.returnsoft.resource.service.InterviewStateInterface;
import com.returnsoft.resource.service.OjtInterface;
import com.returnsoft.resource.service.OjtStateInterface;
import com.returnsoft.resource.service.ProvinceInterface;
import com.returnsoft.resource.service.RecruimentSourceInterface;
import com.returnsoft.resource.service.RecruiterInterface;
import com.returnsoft.resource.service.TrainingInterface;
import com.returnsoft.resource.service.TrainingStateInterface;*/
...
public class CandidateController implements Serializable {
	...
	private CandidateDto candidateSelected;
	...
	public void beforeSearch() {
		...
		try {
			...
			if (areas == null || areas.size() == 0) {
				...
				Integer recruiterId = session.getRecruiterId();
			...
			if (interviewStates == null || interviewStates.size() == 0) {
				/*InterviewStateInterface interviewStateService = (InterviewStateInterface) new InitialContext(
						Util.getInitProperties())
						.lookup("java:global/RecruitmentEAR/RecruitmentEJB/InterviewStateBean"
								+ "!com.returnsoft.resource.service.InterviewStateInterface");
				List<InterviewStateDto> dtos = interviewStateService.findAll();*/
				interviewStates = new ArrayList<SelectItem>();
				/*for (InterviewStateDto dto : dtos) {
					SelectItem item = new SelectItem();
					item.setValue(dto.getId().toString());
			...
			if (trainingStates == null || trainingStates.size() == 0) {
				/*TrainingStateInterface trainingStateService = (TrainingStateInterface) new InitialContext(
						Util.getInitProperties())
						.lookup("java:global/RecruitmentEAR/RecruitmentEJB/TrainingStateBean"
				...
				if (areaSelected.equals("t")) {
					...
					if (areas!=null && areas.size()>0) {
						...
						for (SelectItem areaItem : areas) {
							...
							List<AreaDto> areasDto = areaService.findSubAreasByRecruiter(areaId, recruiterId);
							for (AreaDto areaDto : areasDto) {
								SelectItem item = new SelectItem();
	...
	public void search() {
		...
		try {
				...
				/*CandidateInterface candidateService = (CandidateInterface) new InitialContext(
						Util.getInitProperties())
				...
				if (interviewStateSelected != null
						&& !interviewStateSelected.equals("")) {
					interviewStateId = Integer.parseInt(interviewStateSelected);
				}

				Integer trainingStateId = null;
				if (trainingStateSelected != null
						&& !trainingStateSelected.equals("")) {
				...
				/*candidates = candidateService.findList(areasId,subAreasId,
						...
						scheduledAtFormatter, createdAtFormatter, candidateDocumentNumberSearch,
						candidateNameSearch);*/
	...
	public void beforeAdd() {
		...
		try {
			...
			if (countries == null || countries.size() == 0) {
				/*CountryInterface countryService = (CountryInterface) new InitialContext(
						Util.getInitProperties())
						.lookup("java:global/RecruitmentEAR/RecruitmentEJB/CountryBean"
			...
			if (recruimentSources == null || recruimentSources.size() == 0) {
				/*RecruimentSourceInterface recruimentSourceService = (RecruimentSourceInterface) new InitialContext(
						Util.getInitProperties())
						.lookup("java:global/RecruitmentEAR/RecruitmentEJB/RecruimentSourceBean"
								+ "!com.returnsoft.resource.service.RecruimentSourceInterface");
				List<RecruimentSourceDto> recruimentSourcesDto = recruimentSourceService
						.findAll();*/
				recruimentSources = new ArrayList<SelectItem>();
				/*for (RecruimentSourceDto recruimentSourceDto : recruimentSourcesDto) {
					SelectItem item = new SelectItem();
			...
			if (educationLevels == null || educationLevels.size() == 0) {
				/*EducationLevelInterface educationLevelService = (EducationLevelInterface) new InitialContext(
						Util.getInitProperties())
						.lookup("java:global/RecruitmentEAR/RecruitmentEJB/EducationLevelBean"
								+ "!com.returnsoft.resource.service.EducationLevelInterface");
				List<EducationLevelDto> educationLevelsDto = educationLevelService
						.findAll();*/
				educationLevels = new ArrayList<SelectItem>();
				/*for (EducationLevelDto educationLevelDto : educationLevelsDto) {
					SelectItem item = new SelectItem();
	...
	public void searchDepartments() {
		...
		try {
			...
			if (countryId > 0) {
				/*DepartmentInterface departmentService = (DepartmentInterface) new InitialContext(
						Util.getInitProperties())
						.lookup("java:global/RecruitmentEAR/RecruitmentEJB/DepartmentBean"
	...
	public void searchProvinces() {
		...
		try {
			...
			// /System.out.println(departmentId);
			if (departmentId > 0) {
				/*ProvinceInterface provinceService = (ProvinceInterface) new InitialContext(
						Util.getInitProperties())
						.lookup("java:global/RecruitmentEAR/RecruitmentEJB/ProvinceBean"
				...
				provinces = new ArrayList<SelectItem>();
				/*for (ProvinceDto provinceDto : provincesDto) {
					SelectItem item = new SelectItem();
	...
	public void searchDistricts() {
		...
		try {
			...
			// /System.out.println(departmentId);
			if (provinceId > 0) {
				/*DistrictInterface districtService = (DistrictInterface) new InitialContext(
						Util.getInitProperties())
						.lookup("java:global/RecruitmentEAR/RecruitmentEJB/DistrictBean"
								+ "!com.returnsoft.resource.service.DistrictInterface");
				List<DistrictDto> districtsDto = districtService
						.findByProvince(provinceId);*/
				districts = new ArrayList<SelectItem>();
				/*for (DistrictDto districtDto : districtsDto) {
					SelectItem item = new SelectItem();
	...
	public void calculateAge() {
		try {
			...
			if (candidateSelected != null
					&& candidateSelected.getBornDate() != null) {

				Date current = new Date();
				Long difference = current.getTime()
						- candidateSelected.getBornDate().getTime();
	...
	public String add() {
		try {
			...
			if (recruimentSourceSelected != null
					&& recruimentSourceSelected.length() > 0) {
				Integer recruimentSourceId = Integer
						.parseInt(recruimentSourceSelected);
				RecruimentSourceDto recruimentSource = new RecruimentSourceDto();
			...
			if (educationLevelSelected != null
					&& educationLevelSelected.length() > 0) {
				Integer educationLevelId = Integer
						.parseInt(educationLevelSelected);
			...
			String confirmMessage = "Se creó satisfactoriamente el candidato con id:"
					+ candidateSelected.getId();
			...
			if (interviewStateSelected != null
				...
				InterviewStateDto interviewStateDto = new InterviewStateDto();
				interviewStateDto.setId(Integer
						.parseInt(interviewStateSelected));
				interview.setInterviewState(interviewStateDto);

				/*InterviewInterface interviewService = (InterviewInterface) new InitialContext(
						Util.getInitProperties())
						.lookup("java:global/RecruitmentEAR/RecruitmentEJB/InterviewBean"
	...
	public void beforeEdit(ComponentSystemEvent event) {
		...
		try {
			...
			//if (candidateSelected == null || candidateSelected.getId() == 0) {
				...
				if (candidateIdRequest != null
						&& !candidateIdRequest.equals("")) {
					...
					if (candidateSelected.getArea() != null
							&& candidateSelected.getArea().getId() > 0) {
						if (candidateSelected.getArea().getArea() != null
							...
							for (SelectItem areaItem : areas) {
								...
								if (areaItem.getValue().toString().equals(candidateSelected.getArea().getArea().getId().toString())) {
							...
							if (isAreaPermission) {
								...
								for (SelectItem subAreaItem : subAreas) {
									if (subAreaItem.getValue().toString().equals(candidateSelected.getArea().getId().toString())) {
								...
								if (isSubAreaPermission) {
									...
									//MENSAJE DE ERROR, NO TIENE PERMISOS PARA EL SUBAREA
									FacesMessage msg = new FacesMessage("No tiene permisos para el subarea "+candidateSelected.getArea().getName());
									msg.setSeverity(FacesMessage.SEVERITY_ERROR);
									FacesContext.getCurrentInstance().addMessage(null, msg);
									candidateSelected=null;
								}
								...
								FacesMessage msg = new FacesMessage("No tiene permisos para el area "+candidateSelected.getArea().getArea().getName());
								msg.setSeverity(FacesMessage.SEVERITY_ERROR);
								FacesContext.getCurrentInstance().addMessage(null, msg);
								candidateSelected=null;
							}
					...
					if (isSubAreaPermission && isAreaPermission) {
						...
						if (candidateSelected.getScheduledAt() != null) {
							...
							/*if (interviewsFound != null
									&& interviewsFound.size() > 0) {
								isPending = Boolean.TRUE;
								interviewedAt = interviewsFound.get(0)
										.getInterviewedAt();
								interviewStateSelected = interviewsFound.get(0)
										.getInterviewState().getId().toString();
							} else {

								// VERIFICA CAPACITACION EN PROCESO
								TrainingInterface trainingService = (TrainingInterface) new InitialContext(
										Util.getInitProperties())
								...
								if (trainingsFound != null
										&& trainingsFound.size() > 0) {
									isPending = Boolean.TRUE;
									trainingAt = trainingsFound.get(0)
											.getTrainingAt();
									...
									interviewStateSelected = trainingsFound.get(0)
											.getTrainingState().getId().toString();

								} else {
									// VERIFICA OJT EN PROCESO
									OjtInterface ojtService = (OjtInterface) new InitialContext(
											Util.getInitProperties())
											.lookup("java:global/RecruitmentEAR/RecruitmentEJB/OjtBean"
													+ "!com.returnsoft.resource.service.OjtInterface");
									List<OjtDto> ojtsFound = ojtService
											.findPendingsByCandidateId(candidateId);
									if (ojtsFound != null && ojtsFound.size() > 0) {
										isPending = Boolean.TRUE;
										ojtAt = ojtsFound.get(0).getOjtAt();
										...
										interviewStateSelected = ojtsFound.get(0)
												.getOjtState().getId().toString();
						...
						if (candidateSelected.getCountry() != null
								&& candidateSelected.getCountry().getId() > 0) {
							// SELECCIONA PAIS
							System.out.println("countrySelected:"
									+ candidateSelected.getCountry().getId());
							countrySelected = candidateSelected.getCountry()
									.getId().toString();
							// BUSCA DEPARTAMENTOS
							searchDepartments();
							if (candidateSelected.getDepartment() != null
								...
								departmentSelected = candidateSelected
								...
								if (candidateSelected.getProvince() != null
									...
									provinceSelected = candidateSelected
											.getProvince().getId().toString();
									// BUSCA DISTRITOS
									searchDistricts();
									if (candidateSelected.getDistrict() != null
											&& candidateSelected.getDistrict()
													.getId() > 0) {
										// SELECCIONA DISTRITO
										districtSelected = candidateSelected
												.getDistrict().getId().toString();
									}
								}
						...
						if (candidateSelected.getRecruimentSource() != null
							...
							recruimentSourceSelected = candidateSelected
									.getRecruimentSource().getId().toString();
							onChangeRecruitmentSource();
						}

						if (candidateSelected.getEducationLevel() != null
								&& candidateSelected.getEducationLevel().getId() > 0) {
							educationLevelSelected = candidateSelected
									.getEducationLevel().getId().toString();
						}
					}
					...
					/*if (candidateSelected.getArea() != null
							&& candidateSelected.getArea().getId() > 0) {
						areaSelected = candidateSelected.getArea().getArea()
								.getId().toString();
						searchSubAreas();
						subAreaSelected = candidateSelected.getArea().getId()
								.toString();
					}*/
	...
	public String edit() {
		try {
			...
			if (candidateSelected != null && candidateSelected.getId() > 0) {
				...
				String confirmMessage = "Se editó satisfactoriamente el postulante con id:"
						+ candidateSelected.getId();
				...
				if (interviewStateSelected != null
					...
					/*InterviewInterface interviewService = (InterviewInterface) new InitialContext(
							...
							.lookup("java:global/RecruitmentEAR/RecruitmentEJB/InterviewBean"
									+ "!com.returnsoft.resource.service.InterviewInterface");
					List<InterviewDto> interviewsFound = interviewService
							.findPendingsByCandidateId(candidateSelected
									.getId());*/

					/*if (interviewsFound == null || interviewsFound.size() == 0) {
						// VERIFICA CAPACITACION EN PROCESO
						TrainingInterface trainingService = (TrainingInterface) new InitialContext(
								Util.getInitProperties())
								.lookup("java:global/RecruitmentEAR/RecruitmentEJB/TrainingBean"
						...
						if (trainingsFound == null
							...
							if (ojtsFound == null || ojtsFound.size() == 0) {
								...
								interview.setCreatedAt(new Date());
								interview.setInterviewedAt(interviewedAt);
								InterviewStateDto interviewState = new InterviewStateDto();
								interviewState.setId(Integer
										.parseInt(interviewStateSelected));
	...
	public void onChangeRecruitmentSource(){
		try {
			...
			RecruimentSourceInterface recruimentSourceService = (RecruimentSourceInterface) new InitialContext(
					...
					.lookup("java:global/RecruitmentEAR/RecruitmentEJB/RecruimentSourceBean"
							+ "!com.returnsoft.resource.service.RecruimentSourceInterface");
			RecruimentSourceDto recruimentSourcesDto = recruimentSourceService.findById(recruitmentSourceId);
			*/
	...
	public CandidateDto getCandidateSelected() {
		return candidateSelected;
	}

	public void setCandidateSelected(CandidateDto candidateSelected) {
	...
	public void setCandidateDocumentNumberSearch(
			String candidateDocumentNumberSearch) {
